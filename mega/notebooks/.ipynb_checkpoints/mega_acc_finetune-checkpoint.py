# *********** Finetune abstRCT dataset for ACC and ARIC tasks. ***************************
...
abst_rct_dir = current_dir.parent.absolute()
parent_dir = abst_rct_dir.parent.absolute()
sys.path.append(os.path.abspath(abst_rct_dir))
...
parser.add_argument("task", help="The pipeline task to execute.", type=str, choices=["acc", "ari", "arc", "mega"])
...
DATASET_DIR = os.path.join(abst_rct_dir, "datasets")
LLAMA_FACTORY_DIR = os.path.join(ROOT_DIR, "LLaMA-Factory")
OUTPUT_DIR = os.path.join(abst_rct_dir, "finetuned_models", f"""{TASK}_{BASE_MODEL.split("/")[1]}""")
...
train_dataset_name = f"""mega_acc_train.json"""
train_dataset_file = os.path.join(DATASET_DIR, train_dataset_name)

# *** TEST DATASET *** #

# test_dataset_name = f"""abstRCT_{TASK}_test.json"""
...
if not os.path.exists(os.path.join(abst_rct_dir, "ft_arg_files")):
    os.mkdir(os.path.join(abst_rct_dir, "ft_arg_files"))

# *** TRAIN FILE/DATASET INFO FILE ***

train_file = os.path.join(abst_rct_dir, "ft_arg_files", f"""{train_dataset_name.split(".")[0].split("train")[0]}{BASE_MODEL.split("/")[1]}.json""")
...
data["joint_acc"] = dataset_info_line
...
args = dict(
  ...
  dataset="joint_acc",           # use alpaca and identity datasets
...
for test_set in ['neo', 'gla', 'mix']:
    ...
    test_dataset_name = f"""abstRCT_acc_test_{test_set}.json"""
    test_dataset_file = os.path.join(DATASET_DIR, test_dataset_name)
    ...
    with open(os.path.join(OUTPUT_DIR, f"""abstRCT_{TASK}_results_{NB_EPOCHS}_{test_set}.pickle"""), 'wb') as fh:
...
test_dataset_name = f"""CDCP_acc_test.json"""
test_dataset_file = os.path.join(DATASET_DIR, test_dataset_name)
...
test_dataset_name = f"""PE_acc_essay_wo_tags_test.json"""
test_dataset_file = os.path.join(DATASET_DIR, test_dataset_name)
...
#     elif TASK == 'aric':
#         nr_acs_l = compute_acs_nr(test_set, DATASET_DIR)
#         task_grounds, task_preds = post_process_aric(results, nr_acs_l)
