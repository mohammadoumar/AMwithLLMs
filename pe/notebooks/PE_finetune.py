# *********** Finetune PE dataset for ACC, ARI and ARC tasks. ***************************
...
pe_dir = current_dir.parent.absolute()
parent_dir = pe_dir.parent.absolute()
sys.path.append(os.path.abspath(pe_dir))
...
parser.add_argument("context", help="Context of the task.", type=str, choices=["essay", "paragraph"])
...
DATASET_DIR = os.path.join(pe_dir, "datasets")
LLAMA_FACTORY_DIR = os.path.join(ROOT_DIR, "LLaMA-Factory")
OUTPUT_DIR = os.path.join(pe_dir, "finetuned_models", f"""PE_{TASK}_{CONTEXT}_{TAGS}_{BASE_MODEL.split("/")[1]}""")
...
if not os.path.exists(os.path.join(pe_dir, "ft_arg_files")):
    os.mkdir(os.path.join(pe_dir, "ft_arg_files"))

# *** TRAIN FILE/DATASET INFO FILE ***

train_file = os.path.join(pe_dir, "ft_arg_files", f"""{train_dataset_name.split(".")[0].split("train")[0]}{BASE_MODEL.split("/")[1]}.json""")
